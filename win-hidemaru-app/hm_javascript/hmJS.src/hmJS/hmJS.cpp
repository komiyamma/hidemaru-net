...
static bool BindDllHandle() {
	...
	if (Hidemaru_GetDllFuncCalledType) {
		...
		IJSStaticLib::BindDllHandle((IntPtr)dll);
		return true;
	}
	return false;
}
...
MACRO_DLL intHM_t SetTmpVar(const void* dynamic_value) {
	...
	if (param_type == DLLFUNCPARAM_WCHAR_PTR) {
		return (intHM_t)IJSStaticLib::SetTmpVar(gcnew String((wchar_t *)dynamic_value));
	}
	else {
		return (intHM_t)IJSStaticLib::SetTmpVar((intHM_t)dynamic_value);
	}
}

MACRO_DLL intHM_t PopNumVar() {
	intHM_t num = (intHM_t)IJSStaticLib::PopTmpVar();
	return num;
}

static wstring strvarsopop;
MACRO_DLL const TCHAR * PopStrVar() {
	auto var = (String ^)IJSStaticLib::PopTmpVar();
...
MACRO_DLL intHM_t DoString(const TCHAR *szexpression) {
	...
	return (intHM_t)IJSStaticLib::DoString(gcnew String(szexpression));
}
...
MACRO_DLL intHM_t DoFile(const TCHAR *szfilename) {
	...
	return (intHM_t)IJSStaticLib::DoFile(gcnew String(szfilename));
}
